...
export default function About() {
  ...
  return (
    <section id="about" className="relative py-24 overflow-hidden selection:bg-accent/30">
      {/* Background Blobs */}
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        >
          ...
          <motion.div variants={itemVariants} className="text-center space-y-4">
            <span className="text-accent text-xs font-mono tracking-[0.4em] uppercase">About Me</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
            <motion.div variants={itemVariants} className="space-y-6">
              ...
              <h3 className="text-3xl font-bold">Hi, I'm Anujith KR</h3>
            ...
            <motion.div variants={itemVariants} className="relative group">
              <div className="absolute -inset-4 bg-gradient-to-r from-accent to-purple-600 rounded-2xl blur opacity-20 group-hover:opacity-30 transition duration-500"></div>
              <div className="relative glass-card rounded-2xl p-8 space-y-8">
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-muted">Quick to adapt to new stacks and technologies.</p>
                  </div>
                  <div className="space-y-2">
                    <div className="p-2 w-fit rounded-lg bg-accent/10 text-accent">
                    ...
                    <p className="text-sm text-muted">Love for clean code and beautiful user interfaces.</p>
                  </div>
                </div>
                <p className="text-muted italic border-l-2 border-accent pl-4">
                  "Apart from technology, I’m a disciplined and hardworking person who likes taking on new challenges and continuously growing both personally and professionally."
                </p>
              </div>
            </motion.div>
          </div>
          ...
          <div className="space-y-16">
            ...
            <div className="relative border-l border-muted/20 ml-4 md:ml-0 md:before:absolute md:before:left-1/2 md:before:h-full md:before:w-px md:before:bg-muted/20 md:border-none">
              {timeline.map((item, index) => (
                ...
                >
                  <div className="absolute left-[-17px] md:left-1/2 md:-ml-4 top-0 w-8 h-8 rounded-full bg-background border-2 border-accent flex items-center justify-center z-10">
                    <div className="text-accent">{item.icon}</div>
                  </div>

                  <div className="md:w-[45%] pl-8 md:pl-0">
                    <div className={`glass-card p-6 rounded-2xl hover:border-accent/50 transition-all duration-300 group ${
                      index % 2 === 0 ? 'md:text-left' : 'md:text-right'
                    }`}>
                      <div className={`flex flex-col ${index % 2 === 0 ? '' : 'md:items-end'}`}>
                        <span className="text-accent font-mono text-sm mb-2">{item.date}</span>
                        <h4 className="text-xl font-bold group-hover:text-accent transition-colors">{item.title}</h4>
                        <p className="text-foreground/80 font-medium mb-1">{item.organization}</p>
                        ...
                        <div className={`flex flex-wrap gap-2 ${index % 2 === 0 ? '' : 'md:justify-end'}`}>
                          {item.tags.map(tag => (
                            <span key={tag} className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full bg-secondary text-muted font-bold">
                              {tag}
                            </span>
                          ))}
                        </div>
          ...
          {/* Goal Section */}
          <motion.div variants={itemVariants} className="text-center py-16 px-8 rounded-3xl bg-accent/5 border border-accent/10 relative overflow-hidden group">
            ...
            <h3 className="text-2xl md:text-3xl font-bold mb-10 italic max-w-4xl mx-auto leading-tight text-foreground/90">
            ...
            <div className="flex justify-center">
              <Link 
                ...
                target="_blank"
                className="button-gradient px-8 py-4 rounded-2xl font-bold flex items-center gap-3 transition-transform hover:scale-105 active:scale-95"
              >
